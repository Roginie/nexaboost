...
const BASE_CONHECIMENTO = [
  ...
  { categoria: 'Windows & energia', dificuldade: 'fácil', impacto: 'médio',
    ...
    desc: 'Em Configurações > Jogos > Modo de Jogo, ative a opção. Isso evita que o Windows Update e notificações atrapalhem o desempenho durante a partida. Ou cole o comando abaixo.',
    comando: 'reg add "HKCU\\Software\\Microsoft\\GameBar" /v AllowAutoGameMode /t REG_DWORD /d 1 /f',
    condicao: () => true },
  { categoria: 'Windows & energia', dificuldade: 'fácil', impacto: 'médio',
    titulo: 'Reduzir efeitos visuais do Windows',
    desc: 'Em "Ajustar a aparência e o desempenho do Windows" (pesquise no menu Iniciar), marque "Ajustar para obter um melhor desempenho". Libera RAM e processamento de vídeo. Ou cole o comando abaixo.',
  ...
  { categoria: 'Windows & energia', dificuldade: 'médio', impacto: 'médio',
    ...
    desc: 'Em Configurações > Sistema > Tela > Gráficos, ative essa opção. Reduz a latência entre CPU e GPU em placas mais recentes. Ou cole o comando abaixo no Prompt de Comando como administrador (precisa reiniciar o PC depois).',
    comando: 'reg add "HKLM\\SYSTEM\\CurrentControlSet\\Control\\GraphicsDrivers" /v HwSchMode /t REG_DWORD /d 2 /f',
  ...
  { categoria: 'Drivers e software', dificuldade: 'médio', impacto: 'baixo',
    ...
    desc: 'Antivírus de terceiros que escaneiam arquivos em tempo real podem causar engasgos ao carregar texturas. Adicione a pasta do jogo como exceção (mantenha a proteção geral ativa). Troque o caminho abaixo pelo da pasta do jogo e cole no PowerShell como administrador (não funciona no cmd clássico).',
    comando: 'Add-MpPreference -ExclusionPath "C:\\CAMINHO\\DA\\PASTA\\DO\\JOGO"',
  ...
  { categoria: 'Armazenamento', dificuldade: 'fácil', impacto: 'médio',
    ...
    desc: 'Discos quase cheios (especialmente SSDs) ficam mais lentos para gravar arquivos temporários e podem causar engasgos. Copie o comando abaixo pra ver o espaço livre de cada disco.',
    comando: 'wmic logicaldisk get caption,freespace,size',
    condicao: () => true },
  ...
  { categoria: 'Armazenamento', dificuldade: 'médio', impacto: 'médio',
    ...
    desc: 'Se o jogo está no HDD, desfragmentar de vez em quando ajuda a reduzir os tempos de carregamento. Atenção: nunca desfragmente um SSD/NVMe, isso só desgasta ele à toa. Ajuste a letra do disco abaixo se não for C:.',
    comando: 'defrag C: /O',
    condicao: s => s.storage === 'hdd' },
  ...
  { categoria: 'Rede', dificuldade: 'médio', impacto: 'médio',
    ...
    desc: 'Isso evita que o software de segurança inspecione cada pacote de rede do jogo, o que pode causar picos de ping (spikes). Troque nome/caminho abaixo e cole no Prompt de Comando como administrador.',
    comando: 'netsh advfirewall firewall add rule name="NomeDoJogo" dir=in action=allow program="C:\\CAMINHO\\jogo.exe" enable=yes',
    condicao: s => s.jogoTipo === 'online' },
